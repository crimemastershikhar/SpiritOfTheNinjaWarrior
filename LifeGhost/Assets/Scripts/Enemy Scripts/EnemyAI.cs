...
public class EnemyAI : MonoBehaviour {
    ...
    void EnemyTrackingPlayer() {
        ...
        if (enemyHealth.realHealth > 0) {
            ...
            col.enabled = false;    //Playing dead anim from enemy 
        }

        if (playerHealth.realHealth <= 0)
            EnemyVictory();       //Play vic anim

        if (victory) {
            StopVictoryAnimation();    //Stop vic anim
